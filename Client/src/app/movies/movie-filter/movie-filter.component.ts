...
export class MovieFilterComponent implements OnInit {
  ...
  filterMovies(values: any){
    values.page = this.curentPage;
    values.recordsPerPage = this.recordsPerPage;
    console.log(values)
    this.moviesService.filter(values).subscribe((response: any)=>{
      this.movies = response.body;
      this.totalAmountOfRecords = response.headers.get("totalAmountOfRecords")
    })
  }

  private readParametarFromURL(){
  ...
  private writeParametarsInURL(){
    ...
    queryStrings.push(`page=${this.curentPage}`);
